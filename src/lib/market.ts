...
import { recordBoard } from "./signals.ts";
...
export async function fetchBoard(): Promise<MarketView[]> {
  ...
  // Signal log: fire-and-forget. recordBoard samples at most one row per
  // market per 60s (internal gate), caps the file at a ring buffer, degrades
  // to in-memory on read-only FS, and never throws — recording must never
  // block or break the live board.
  void recordBoard(views, now).catch(() => {});
  return views;
}
